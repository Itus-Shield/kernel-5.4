...
platform_do_flash() {
	...
	if [ $board = "itus,shield-router" ]; then
		...
		cp "/tmp/$board_dir/kernel" /boot/$kernel
		umount /boot
		echo "flashing rootfs to ${rootfs}"
		dd if="/tmp/$board_dir/root" of="${rootfs}"
	else
		...
		echo "flashing rootfs to ${rootfs}"
		tar xvf $tar_file $board_dir/root -O | dd of="${rootfs}" bs=4096
	fi
	sync
}
...
platform_check_image() {
	...
	itus,shield-router)
		local tar_file="$1"
		local kernel_length=$(tar xvf $tar_file $board_dir/kernel -O | wc -c 2> /dev/null)
		local rootfs_length=$(tar xvf $tar_file $board_dir/root -O | wc -c 2> /dev/null)
